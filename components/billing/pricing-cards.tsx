...
export function PricingCards({
  ...
  function checkDowngradeLimits(planKey: PlanKey): string[] {
    const plan = PLANS[planKey]
    const issues: string[] = []

    if (plan.maxRepos !== -1 && currentRepoCount > plan.maxRepos) {
      issues.push(
        `You have ${currentRepoCount} repos but the ${plan.name} plan allows only ${plan.maxRepos}.`
      )
    }
    if (plan.maxMembers !== -1 && currentMemberCount > plan.maxMembers) {
      issues.push(
        `You have ${currentMemberCount} members but the ${plan.name} plan allows only ${plan.maxMembers}.`
      )
    }
  ...
  async function handleSubscribe(planKey: PlanKey) {
    ...
    const issues = checkDowngradeLimits(planKey)
    if (issues.length > 0) {
      setDowngradeWarning({ planKey, issues })
      return
    }

    await proceedWithCheckout(planKey)
  }

  async function proceedWithCheckout(planKey: PlanKey) {
    const plan = PLANS[planKey]
    if (!plan.stripePriceId) return
      ...
      {downgradeWarning && (
        <div className="mb-6 rounded-lg border border-amber-300 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">
            Switching to this plan may cause issues:
          </p>
          <ul className="mt-2 list-inside list-disc text-sm text-amber-700 dark:text-amber-300">
            {downgradeWarning.issues.map((issue) => (
              <li key={issue}>{issue}</li>
            ))}
          </ul>
          <div className="mt-3 flex gap-2">
            <button
              onClick={() => proceedWithCheckout(downgradeWarning.planKey)}
            ...
            <button
              ...
              className="rounded-lg border border-amber-300 px-3 py-1.5 text-sm font-medium text-amber-800 hover:bg-amber-100 dark:border-amber-700 dark:text-amber-200 dark:hover:bg-amber-900"
            >
              Cancel
            </button>
          </div>
